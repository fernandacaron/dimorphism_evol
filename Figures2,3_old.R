...
sdi <- sdi[names(sdi) %in% pam_crop$Species_name]
sdi <- sdi[pam_crop$Species_name]
...
res1 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q1])
res2 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q2])
res3 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q3])
res0 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q0])
res4 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q4])
res5 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q5])
res6 <- lets.subsetPAM(pam_crop, pam_crop[[3]][pam_crop[[3]] %in% q6])

rasterize.ssd <- function(pam, sdi) {
    sdi[sdi == 0] <- 1e-38
    ...
    res <- cbind(pam[[1]][, 1:2], summ)
    res2 <- na.omit(res)
    colnames(res)[3] <- paste("Variable", 
                              as.character(substitute(median)), sep = "_")
    r <- rasterize(res2[, 1:2], pam[[2]], res2[, 3])
}

r <- rasterize.ssd(pam_crop, sdi)
r1 <- rasterize.ssd(res1, sdi[names(sdi) %in% res1[[3]]])
r2 <- rasterize.ssd(res2, sdi[names(sdi) %in% res2[[3]]])
r3 <- rasterize.ssd(res3, sdi[names(sdi) %in% res3[[3]]])
r0 <- rasterize.ssd(res0, sdi[names(sdi) %in% res0[[3]]])
r4 <- rasterize.ssd(res4, sdi[names(sdi) %in% res4[[3]]])
r5 <- rasterize.ssd(res5, sdi[names(sdi) %in% res5[[3]]])
r6 <- rasterize.ssd(res6, sdi[names(sdi) %in% res6[[3]]])


sdi_95 <- sdi[sdi <= quantile(sdi, probs = 0.975) &
                sdi >= quantile(sdi, probs = 0.025)]
res95 <- lets.subsetPAM(pam_crop, 
                        pam_crop[[3]][pam_crop[[3]] %in% names(sdi_95)])
r95 <- rasterize.ssd(res95, sdi_95[names(sdi_95) %in% res95[[3]]])

pdf("figures/FigureXX2.pdf", width = 20, height = 8)
...
map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r, col = pal(200), add = TRUE)
mtext("SSD", side = 3, line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 5))
plot(r1, col = pal(200), add = TRUE)
mtext(paste0("SDI = ", round(min(sdi), 2), " - ", round(q[1], 2)), side = 3,
      line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r2, col = pal(200), add = TRUE)
mtext(paste0("SDI = ", round(q[1], 2), " - ", round(q[2], 2)), side = 3,
      line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r3, col = pal(200), add = TRUE)
mtext(paste0("SDI = ", round(q[2], 2), " - ", round(q[3], 2)), side = 3,
      line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 5))
plot(r0, col = pal(200), add = TRUE)
mtext("SDI = 0", side = 3, line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r4, col = pal(200), add = TRUE)
mtext(paste0("SDI = ", round(q[3], 2), " - ", round(q[4], 2)), side = 3, 
      line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r5, col = pal(200), add = TRUE)
mtext(paste0("SDI = ", round(q[4], 2), " - ", round(q[5],2)), side = 3,
      line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r6, col = pal(200), add = TRUE)
mtext(paste0("SDI = ", round(q[5], 2), " - ", round(max(sdi), 2)), side = 3,
      line = -1.5, adj = 0, cex = 0.9)

map("world", fill = TRUE, col = "gray", bg = "white", border = NA, 
    mar = c(0, 0, 0, 4))
plot(r95, col = pal(200), add = TRUE)
mtext("95% SSD", side = 3, line = -1.5, adj = 0, cex = 0.9)
